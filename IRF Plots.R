...
allirfs <- list(main = irf.main.ortho.1, exog = irf.exog.ortho.1, pre = irf.pre.ortho,post = irf.post.ortho, assets = irf.assets.ortho)
mainfolder <- "~/Thesis/Figures and Graphs/"
subfolders <- c("main irf", "exog irf", "pre irf", "post irf", "assets irf")
folders <- paste(mainfolder, subfolders, sep = "")
savelocation <- list()
for(i in 1:(length(folders)-1)) {
  savelocation[[i]] <- paste(folders[i], "/", var.names.main, ".png", sep = "")
}
savelocation[[5]] <- paste(folders[5], "/", var.names.assets, ".png", sep = "")

for(j in 1:(length(folders)-1)) {
...
for(i in 1:length(var.names.assets)) {
  png(savelocation[[5]][[i]])
  bazplotirf(allirfs[[5]][[i]], plot.type = "multiple", ylab = var.names.fancy.main)
  dev.off()
}
