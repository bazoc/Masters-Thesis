          ...
          ...) 
{
  ...
  irs <- vars:::.irf(x = x, impulse = impulse, response = response, 
  ...
  if (boot) {
    ...
    ci <- 1 - ci
    BOOT <- baz.boot(x = x, n.ahead = n.ahead, runs = runs, 
...
bazirf.svarest <- function (x, impulse = NULL, response = NULL, n.ahead = 10, ortho = TRUE, 
                            cumulative = FALSE, boot = TRUE, ci = 0.95, runs = 100, seed = NULL, 
                            impnamesfancy = NULL, resnamesfancy = NULL) 
{
  if (!(class(x) == "svarest")) {
