...
baz.boot <- function (x, n.ahead, runs, ortho, cumulative, impulse, response, 
          ci, seed, y.names) 
{
  if (!(is.null(seed))) 
    set.seed(abs(as.integer(seed)))
  if (class(x) == "varest") {
    #This was originally exaluate
    VAR <- x
  }
  ...
  for (i in 1:runs) {
    ...
    BOOT[[i]] <- vars:::.irf(x = varboot, n.ahead = n.ahead, ortho = ortho, 
...
saywhatyousay <- function (x, n.ahead, runs, ortho, cumulative, impulse, response, 
          ci, seed, y.names) 
{
  if (!(is.null(seed))) 
    set.seed(abs(as.integer(seed)))
