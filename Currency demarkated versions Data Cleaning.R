...
unique(BOI$Subject)
sm <- subset(BOI,
             Subject == "National Accounts > National Accounts Deflators > Gross Domestic Product > GDP Deflator")
unique(sm$Measure)
temp <- filter(BOI, Subject == "Share Prices > All shares/broad > Total > Total" & Country == "Austria")
BOI$var <- NULL 
for(i in 1:nrow(BOI)) {
  if(BOI$Subject[i] == "Share Prices > All shares/broad > Total > Total" & BOI$Measure[i] == "Index 2015=100") BOI$var[i] <- "Share Prices"
  if(BOI$Subject[i] == "National Accounts > GDP by Expenditure > Constant Prices > Gross Domestic Product - Total" & BOI$Measure[i] == "Level, rate or national currency, s.a.") BOI$var[i] <- "Real GDP, s.a"
  ...
  if(BOI$Subject[i] == "National Accounts > National Accounts Deflators > Gross Domestic Product > GDP Deflator") BOI$var[i] <- "GDP Deflator, s.a"
  if(BOI$Subject[i] == "National Accounts > GDP by Expenditure > Constant Prices > Gross Fixed Capital Formation" & BOI$Measure[i] == "Level, rate or national currency, s.a.") BOI$var[i] <- "Real GFCF, s.a"
...
for(i in 1:nrow(BOI)) {
  country <- subset(ind2015,
                    Country == BOI2015[i,2])
  BOI2015[i,8:17] <- (BOI[i,8:18]*100) / country[,8:18]
}
